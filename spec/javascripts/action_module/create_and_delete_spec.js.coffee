	...
	describe "Action manager should", ->
		...
		describe "undo last items on list, and add to '_redoStack'", ->
			...
			describe "with correct properties.", ->
				Then -> expect(App.Action._getUndoneHistory()[0]['type']).toEqual('deleteNote')
				And -> expect(App.Action._getUndoneHistory()[0]['changes']['note']['guid']).toEqual(jasmine.any(String))
				And -> expect(App.Action._getUndoneHistory()[0]['changes']['note']['depth']).toEqual(jasmine.any(Number))
				And -> expect(App.Action._getUndoneHistory()[0]['changes']['note']['rank']).toEqual(jasmine.any(Number))
				And -> expect(App.Action._getUndoneHistory()[0]['changes']['note']['parent_id']).toEqual(jasmine.any(String))
				And -> expect(App.Action._getUndoneHistory()[0]['changes']['note']['title']).toEqual(jasmine.any(String))
				And -> expect(App.Action._getUndoneHistory()[0]['changes']['note']['subtitle']).toEqual(jasmine.any(String))
				And -> expect(App.Action._getUndoneHistory()[0]['changes']['note']['created_at']).toEqual(jasmine.any(String))
				And -> expect(App.Action._getUndoneHistory()[0]['changes']['note']['id']).toEqual(jasmine.any(Number))
